...
- `WA_WEB_SESSIONS_DIR=/var/lib/whatsapp-bridge/sessions` (or another persistent path)
...
## 2) Persistence
- Auth state is stored on disk (encrypted with `WA_WEB_AUTH_SECRET`). Use a persistent volume/path so users don’t have to rescan QR codes after restart.
...
## 5) Network & security
- Restrict ingress to backend IPs/VPC; the service rejects requests without `WA_WEB_BRIDGE_API_KEY`.
- Keep `WA_WEB_AUTH_SECRET` secret; it encrypts auth blobs stored on disk.
